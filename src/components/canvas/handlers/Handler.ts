...
export interface HandlerOption {
   ...
   * @type {FabricCanvas}
   */
  canvas?: FabricCanvas<PlotPlanCanvas>;
  /**
...
class Handler implements HandlerOptions {
   ...
   * @param {boolean} renderCanvas attempt to assign into current canvas
   * @returns
   */
  public add = (obj: FabricObjectOption, centered = true, loaded = false, renderCanvas = true) => {
    ...
    if (obj.type === 'image') {
      ...
      // fixed add group using new option renderCanvas to make sure this canvas would be assigned into current render context
  ...
  public addGroup = (obj: FabricGroup, centered = true, loaded = false) => {
    return obj.objects.map(child => {
      return this.add(child, centered, loaded, false);
    });
  };
