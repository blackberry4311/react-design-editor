...
import { Handler } from '.';
import { FabricImage, WorkareaImageOption, WorkareaLayout, WorkareaObject } from '../utils';
...
class WorkareaHandler {
   ...
   * @returns
   */
  setImage = async (source: string | File, options?: WorkareaImageOption) => {
    const { loaded, resetScale } = options;
    const { canvas, workarea, editable } = this.handler;
